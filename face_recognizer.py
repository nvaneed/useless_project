...
if __name__ == "__main__":
    try:
        ...
        THRESHOLD = 60  # lower = stricter, higher = more lenient

        # Target detection
        TARGET_NAME = "Navaneeth"
        TARGET_URL = "https://script.google.com/macros/s/AKfycbzjBNPXKcDa2NrizQqOgLgAu959FlrHbtw12FX--QL2D91VJDjhDaqTq9RpupeH2Rf3sA/exec?mode=verify"  # Replace with your URL
        sent_flag = False  # To avoid sending repeatedly
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                # Debug print
                logger.info(f"Predicted ID={id}, Confidence={confidence:.1f}")
                ...
                # Send request if Navaneeth is detected
                if name == TARGET_NAME and not sent_flag:
                    try:
                        response = requests.get(TARGET_URL, timeout=5)
                        logger.info(f"Web request sent for {TARGET_NAME}, Status: {response.status_code}")
                        sent_flag = True
                    except Exception as req_err:
                        logger.error(f"Failed to send request for {TARGET_NAME}: {req_err}")

                # Display results
                cv2.putText(img, name, (x+5, y-5),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                cv2.putText(img, f"{confidence:.1f}", (x+5, y+h-5),
